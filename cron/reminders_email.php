...
<?php 
	...
	//user contacts that need a reminder to login
	$login_reminder_date=new date();
	$login_reminder_date->Add(-10);
	
	$where=array(1);
	$where[]="user_under_contract=0";
	$where[]="user_contact_last_login<".$login_reminder_date->GetTimestamp();
	$where[]="user_contact_last_login_reminder<".$login_reminder_date->GetTimestamp();
	$where[]="user_contact_welcome_timestamp>0";
	$where[]="user_contact_welcome_timestamp<".$login_reminder_date->GetTimestamp();
	$where[]="user_contact_email!=''";
	$where[]="user_active=1";

  	$user_contact_list=new DBRowSetEX('user_contacts','user_contact_id','user_contact',implode(' AND ',$where));
	$user_contact_list->join_tables='users';
	$user_contact_list->join_where='users.user_id=user_contacts.user_id';
	$user_contact_list->Retrieve();
  	$user_contact_list->Each('SendLoginReminder',array($where));
...
//	$where[]="timeline_item_date>='".$date->GetDBDate()."'";
	...
	foreach($timeline_item_list->items as $timeline_item)
	{
  	...
  	$list->Retrieve();
  	$list->Each('SendEmailReminders',array($where));
