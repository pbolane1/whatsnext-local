...
<html lang="en">
<head>
	<title>Agent Manager - Transaction Dates</title>
...
<?php
	$temp=new user($HTTP_GET_VARS['user_id']);
	$agent=new agent($temp->Get('agent_id'));
	...
	else
	{
		...
		$coordinator->EditPropertyInfo($HTTP_GET_VARS);
		echo("</div>");

	 	echo("<div id='user_dates_container'>");
		$coordinator->EditUserDates($HTTP_GET_VARS);
		echo("</div>");
	}
?>		
