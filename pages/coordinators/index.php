...
<?php
	...
	else
	{
		...
		echo("<p>&nbsp;</p><h1 class='agent_color1'>Agent Transactions</h1>");
		echo("<div id='".$coordinator->GetFieldName('ListUsersFiltersContainer')."'>");
		$coordinator->ListUsersFilters();
		echo("</div>");
		echo("<div id='".$coordinator->GetFieldName('ListUsersContainer')."'>");
		$coordinator->ListUsers($HTTP_GET_VARS);
		echo("</div>");
		$coordinator->EditUserArea($HTTP_GET_VARS);		
  	}
?>	
	 	    </div>
	    </div>
