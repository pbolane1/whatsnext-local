...
<?php
	$cntrec=database::fetch_array(database::query("SELECT COUNT(1) AS cnt FROM general_lists WHERE foreign_class='".Session::Get('WYSIWYG_CLASS')."' AND foreign_id='".Session::Get('WYSIWYG_ID')."'"));
	if($HTTP_GET_VARS['action']=='newlist')
	{
		if(!$HTTP_POST_VARS['list_name'])
			$HTTP_POST_VARS['list_name']='List '.($cntrec['cnt']+1);
		$list=new general_list();
		$list->Set('general_list_name',$HTTP_POST_VARS['list_name']);
...
<head>
	<title>Insert Detail List</title>
							...
							<tr>
								...
								<?php									  
									if(!$cntrec['cnt'])
										...
										form::DrawSelectFromSQL('list_id',"SELECT * FROM general_lists WHERE foreign_class='".Session::Get('WYSIWYG_CLASS')."' AND foreign_id='".Session::Get('WYSIWYG_ID')."' ORDER BY general_list_name","general_list_name","general_list_id",$list_id);
								?>
								</td> 
							</tr>
							<!--tr>
