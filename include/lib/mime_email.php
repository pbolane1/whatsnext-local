...
/*
  ***Adapted from osCommerce***
  ...
  mime_email.php - a class to assist in building mime-HTML eMails
...
//define defaults////////////////
//used here and mime.php       //
define('EMAIL_TRANSPORT','smtp');
define('EMAIL_LINEFEED','CRLF');
define('CHARSET','iso-8859-1');
//define tep functions from os commerce used here
  ...
  {
	//disable this function... is double spacing when properly fmt'd w/ \r\n
	return $string;
...
//            $rel =& $this->add_related_part($message);
      ...
      if ( (isset($message)) && (is_object($message)) ) {
		...
		if(is_array($this->headers) and is_array($headers))
 ...
 */
    ...
    function send($to_name, $to_addr, $from_name, $from_addr, $subject = '', $headers = '') {
      ...
      if ($headers and is_string($headers)) {
      ...
      if (EMAIL_TRANSPORT == 'smtp') {
        return mail($to_addr, $subject, $this->output, 'From: ' . $from . ($to?($this->lf . 'To: ' . $to):"") . $this->lf . implode($this->lf, $this->headers) . $this->lf . implode($this->lf, $xtra_headers),$this->return_path?"-f ".$this->return_path:'');
      } else {
        return mail($to, $subject, $this->output, 'From: '.$from.$this->lf.implode($this->lf, $this->headers).$this->lf.implode($this->lf, $xtra_headers),$this->return_path?"-f ".$this->return_path:'');
      }
    }
